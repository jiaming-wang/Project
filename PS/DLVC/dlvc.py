'''
Author: Chen Zhitao 2290445045@qq.com
Date: 2024-05-31 09:36:06
LastEditors: Chen Zhitao 2290445045@qq.com
LastEditTime: 2024-07-23 19:26:08
FilePath: /HSI-MSI-Fusion-main/model/my.py
...
class PixelSNAIL(nn.Module):
    def __init__(
        ...
        self.cond_net = CondResNet(nb_cond*cond_embedding_dim, cond_res_channel, cond_res_kernel, n_out_res_block)
        # create smaller conditioning resnet for all but the largest condition
        self.cond_in_net = nn.ModuleList([
            CondResNet(cond_embedding_dim, cond_embedding_dim, cond_res_kernel, n_cond_res_block) if n_cond_res_block > 0 else nn.Identity()
        for _ in range(nb_cond - 1)])
        self.cond_in_net.append(nn.Identity())
        
        self.inconv = torch.nn.Conv2d(50, 50, 3, 1, 1)
            
    def forward(self, input, condition):

        batch, channel, height, width = input.shape
        # input = (
        #     F.one_hot(input.long(), self.n_class).permute(0, 3, 1, 2).type_as(self.background)
        # )
        ...
        for c in cs:
            test = F.interpolate(c, size=(height, width))
        ...
        for block in self.blocks:
            out = block(out, background, condition=cs)
...
if __name__ == '__main__':       
    ...
    x = torch.FloatTensor(2, 3, 24, 24).random_(0, 255)
    cs = [torch.FloatTensor(2, 64, 6, 6).random_(0, 255), torch.FloatTensor(2, 64, 12, 12).random_(0, 255)]
    ps = PixelSNAIL([24, 24], 50, 64, 5, 4, 2, 32)
    print(ps(x, cs).shape)